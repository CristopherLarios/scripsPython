...
class validarcufes():
    ...
    def conteoCufe(self):
        ...
        try:
            ...
            commandConteo = """
                        select count(*) as cantidad, t.Cufe as Cufe
                        from MDL11.tblElectronicInvoice t
                        inner join MDL11.CUFES_Cochez c on t.Cufe = c.CUFE
                        group by t.Cufe
                    """

            dataConteo = self.instance_sql.queryToPandas(commandConteo)

            print('Begining ################################')
            bar = Bar('Processing', max=dataConteo.shape[0])

            for index, row in dataConteo.iterrows():
                insertConteo = """
                            insert into MDL11.conteoCochez(cantidad, cufe) values ({0},'{1}')
                        """.format(row['cantidad'], row['Cufe'])

                insert = self.instance_sql.executeCommand(insertConteo)
    ...
    def validarCufesDGI(self):
        ...
        try:
            ...
            commadSelectCufes = """
                select  cantidad, cufe, validate,urlQr,inDGI,qrAludra,Auth_Protocol from MDL11.conteoFmotta where validate = 0 AND cufe IS NOT  NULL
            """
                    ...
                    for indexcufe, rowCufe in findCufeALudra.iterrows():
                        ...
                        commandUpdate = """
                                ...
                                """.format(qr, 1 if dgi else 0, 1 if aludra else 0, row['cufe'],
                                           rowCufe['Resolution'],
                                           (rowCufe['Response_Code'] if (rowCufe['Response_Code'] != None and rowCufe[
                                               'Response_Code'] != '') else '')
                                ...
                                doc['rProtFe']['gInfFE']['dProtAut'] if dgi else '') if (
                                        rowCufe['PacResponse'] != '' and rowCufe['PacResponse'] is not None) else '', rowCufe['AmountWithTax'])
